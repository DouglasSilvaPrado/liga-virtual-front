...
export default function CreateCompetitionModal({
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    const match_settings = {
      premio_vitoria: Number((document.getElementById("premio_vitoria") as HTMLInputElement)?.value || 0),
      premio_empate: Number((document.getElementById("premio_empate") as HTMLInputElement)?.value || 0),
      premio_derrota: Number((document.getElementById("premio_derrota") as HTMLInputElement)?.value || 0),
      premio_gol: Number((document.getElementById("premio_gol") as HTMLInputElement)?.value || 0),
      cartao_amarelo: Number((document.getElementById("cartao_amarelo") as HTMLInputElement)?.value || 0),
      cartao_vermelho: Number((document.getElementById("cartao_vermelho") as HTMLInputElement)?.value || 0),
      multa_wo: Number((document.getElementById("multa_wo") as HTMLInputElement)?.value || 0),
      mostrar_gols_artilharia: (document.getElementById("mostrar_gols_artilharia") as HTMLInputElement)?.checked ?? true,
      melhor_jogador_partida: (document.getElementById("melhor_jogador_partida") as HTMLInputElement)?.checked ?? true,
      pontos_vitoria: Number((document.getElementById("pontos_vitoria") as HTMLInputElement)?.value || 3),
      pontos_empate: Number((document.getElementById("pontos_empate") as HTMLInputElement)?.value || 1),
      pontos_derrota: Number((document.getElementById("pontos_derrota") as HTMLInputElement)?.value || 0),
    };
    ...
    if (data.type === "divisao") {
      ...
      typeSettings = {
        qtd_classifica_para_mata: parseInt((document.getElementById("dm_qtd_classifica") as HTMLInputElement)?.value || "0") || null,
        chave_automatica: (document.getElementById("dm_chave_automatica") as HTMLSelectElement)?.value || "melhor_x_pior",
        criterio_mata_mata: (document.getElementById("dm_criterio") as HTMLSelectElement)?.value || "saldo_gols",
        ida_volta: (document.getElementById("dm_ida_volta") as HTMLInputElement)?.checked ?? true,
        mata_em_ida_e_volta: (document.getElementById("dm_mata_ida_volta") as HTMLInputElement)?.checked ?? true,
        final_ida_volta: (document.getElementById("dm_final_ida_volta") as HTMLInputElement)?.checked ?? true,
        disputa_terceiro_quarto: (document.getElementById("dm_terceiro_quarto") as HTMLInputElement)?.checked ?? false,
      };
    } else if (data.type === "copa_grupo") {
      typeSettings = {
        num_grupos: parseInt((document.getElementById("cg_num_grupos") as HTMLInputElement)?.value || "0") || null,
        ida_volta: (document.getElementById("cg_ida_volta") as HTMLInputElement)?.checked ?? true,
      };
    } else if (data.type === "copa_grupo_mata") {
      typeSettings = {
        num_grupos: parseInt((document.getElementById("cgm_num_grupos") as HTMLInputElement)?.value || "0") || null,
        qtd_classifica_por_grupo: parseInt((document.getElementById("cgm_qtd_classifica") as HTMLInputElement)?.value || "0") || null,
        chave_automatica: (document.getElementById("cgm_chave_automatica") as HTMLSelectElement)?.value || "melhor_x_pior",
        criterio_mata_mata: (document.getElementById("cgm_criterio") as HTMLSelectElement)?.value || "saldo_gols",
        ida_volta: (document.getElementById("cgm_ida_volta") as HTMLInputElement)?.checked ?? true,
        mata_em_ida_e_volta: (document.getElementById("cgm_mata_ida_volta") as HTMLInputElement)?.checked ?? true,
        final_ida_volta: (document.getElementById("cgm_final_ida_volta") as HTMLInputElement)?.checked ?? true,
        disputa_terceiro_quarto: (document.getElementById("cgm_terceiro_quarto") as HTMLInputElement)?.checked ?? false,
      };
    } else if (data.type === "mata_mata") {
      typeSettings = {
        criterio_mata_mata: (document.getElementById("mm_criterio") as HTMLSelectElement)?.value || "saldo_gols",
        mata_em_ida_e_volta: (document.getElementById("mm_mata_ida_volta") as HTMLInputElement)?.checked ?? true,
        final_ida_volta: (document.getElementById("mm_final_ida_volta") as HTMLInputElement)?.checked ?? true,
        disputa_terceiro_quarto: (document.getElementById("mm_terceiro_quarto") as HTMLInputElement)?.checked ?? false,
      };
    }
    ...
    <div className="max-h-[70vh] overflow-y-auto p-6"> 
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
          ...
          <div className="mt-6 space-y-4 border rounded-lg p-4">
            <h3 className="font-semibold text-lg">Configurações da Partida (comuns)</h3>
          ...
          {isType("divisao") && (
            <div className="mt-6 space-y-4 border rounded-lg p-4">
              <h3 className="font-semibold text-lg">Configurações — Divisão</h3>
          ...
          {isType("divisao_mata") && (
            <div className="mt-6 space-y-4 border rounded-lg p-4">
              <h3 className="font-semibold text-lg">Configurações — Divisão + Mata-mata</h3>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select id="dm_chave_automatica" className="w-full border rounded p-2">
                ...
                <div>
                  ...
                  <select id="dm_criterio" className="w-full border rounded p-2">
          ...
          {isType("copa_grupo") && (
            <div className="mt-6 space-y-4 border rounded-lg p-4">
              <h3 className="font-semibold text-lg">Configurações — Copa Grupo</h3>
          ...
          {isType("copa_grupo_mata") && (
            <div className="mt-6 space-y-4 border rounded-lg p-4">
              <h3 className="font-semibold text-lg">Configurações — Copa Grupo + Mata-mata</h3>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <Label>Chave automática</Label>
                  <select id="cgm_chave_automatica" className="w-full border rounded p-2">
                    <option value="melhor_x_pior">Melhor x Pior</option>
                    <option value="aleatorio">Aleatório</option>
                  </select>
                </div>
                ...
                <div>
                  <Label>Critério</Label>
                  <select id="cgm_criterio" className="w-full border rounded p-2">
                    <option value="saldo_gols">Saldo de gols</option>
                    <option value="gol_fora">Gol fora</option>
                  </select>
                </div>
          ...
          {isType("mata_mata") && (
            <div className="mt-6 space-y-4 border rounded-lg p-4">
              <h3 className="font-semibold text-lg">Configurações — Mata-mata</h3>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label>Critério</Label>
                  <select id="mm_criterio" className="w-full border rounded p-2">
                    <option value="saldo_gols">Saldo de gols</option>
                    <option value="gol_fora">Gol fora</option>
                  </select>
                </div>
          ...
          <div className="flex items-center justify-end space-x-3 pt-4">
            ...
            <Button type="submit" disabled={loading}>{loading ? "Criando..." : "Criar competição"}</Button>
          </div>
         </form>
    </div>
  </DialogContent>
